...
function createApp(options = {}) {
    ...
    function publishRoster() {
        if (!cfg.mirrorUrl || cfg.replica) return;
        store.enqueueOutbox({ type: 'roster', roster: roster() });
    }
    async function flushOutbox() {
        if (!cfg.mirrorUrl) return { sent: 0, remaining: 0 };
        let sent = 0;
        for (const item of store.peekOutbox(20)) {
            try {
                const res = await cfg.fetch(new URL('/api/mirror', cfg.mirrorUrl), {
                    ...
                    body: JSON.stringify(item.payload), signal: AbortSignal.timeout(5000)
                });
                if (!res.ok) throw new Error(`mirror answered ${res.status}`);
                store.ackOutbox(item.seq); sent++;
            } catch { store.failOutbox(item.seq); break; }
        }
        return { sent, remaining: store.outboxDepth() };
    }
    ...
    async function handle(req, res) {
        ...
        try {
            if (p === '/api/health' && m === 'GET') {
                return send(res, 200, { ok: true, version: '3.0.0', replica: cfg.replica, startedAt, outbox: store.outboxDepth(), serverTime: new Date().toISOString() });
            }
            ...
            if (p === '/api/export' && m === 'GET') {
                ...
                return send(res, 200, lines.join('\r\n'), 'text/csv; charset=utf-8', { 'Content-Disposition': `attachment; filename="signin-log-${localDay(new Date().toISOString())}.csv"` });
            }
    ...
    return { handle, store, cfg, flushOutbox, roster, localDayStart, fireStart, close: () => store.close() };
}
